...
def transform_cine_data(csv, txt):
    ...
    for column in df_cine.columns:
        if column in ['university', 'career', 'name', 'location']:
            ...
            df_cine['gender'] = df_cine['gender'].apply(lambda x: x.lower().replace('m', 'male').replace('f', 'female').strip(' '))
        elif column == 'inscription_date':
            df_cine['inscription_date'] = df_cine['inscription_date'].apply(lambda x: datetime.strftime(datetime.strptime(x, '%d-%m-%Y'), '%Y-%m-%d'))
...
def transform_uba_data(csv, txt):
    ...
    for column in df_uba.columns:
        if column in ['university', 'career', 'name', 'location']:
            ...
            df_uba['gender'] = df_uba['gender'].apply(lambda x: x.replace('m', 'male').replace('f', 'female').strip(' '))
        elif column == 'inscription_date':
            df_uba['inscription_date'] = df_uba['inscription_date'].apply(lambda x: datetime.strftime(datetime.strptime(x, '%d-%b-%y'), '%Y-%m-%d'))
