...
if __name__ == '__main__':
    ...
    logfilename = path.join(path.dirname
                            (path.abspath(__file__)), 'logs', 'log_top100.log')
    ...
    # Map questions
    mapped_questions = list(map(mapper_questions, data_chunk1))
    ...
    # Reduce to obtain average response
    average_reponses = reduce(lambda i, j: i + j, mapped_qa)/len(mapped_qa)

    # Save in log file
    logger_dev.info(f'Result average response time v1: {average_reponses}')

    # Obtain the run time and saved log file
    logger_dev.info(f'Runtime v1: {str(time.process_time() - start)}s')
